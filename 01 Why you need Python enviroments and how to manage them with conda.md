...
环境由特定的Python版本和一些包组成。因此，如果您想要**开发或使用具有不同Python或包版本需求的应用程序**，您需要设置不同的环境。
...
3. **灵活性**:它包含很多包(PIP包也可以安装到Conda环境中)
4. **多用途**:它不仅用于管理Python环境和包——您还可以将其用于R(用于统计计算的编程语言)

在撰写本文时，我使用的是4.3.x版本的Conda，但是也有新的4.4.x版本。
...
在Conda环境中最重要的**可执行文件**和目录(放置在```\envs\environmentname```目录)中:
